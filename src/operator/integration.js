    ...
    return kity.createClass( 'IntegrationOperator', {
        ...
        applyOperand: function ( exp, sup, sub ) {
            ...
            var opShape = this.getOperatorShape(),
                ...
                space = new ScriptController( this, opShape, sup, sub, this.posiOp).applySide(),
        ...
        getEllipse:function(group){
            ...
            var info= group.getRenderBox(),
            ratio=3/8,
            size=0.4;

            var cx= (info.x+info.width)/2,
                cy= (info.y+info.height)/2,
                rx=info.width*size,
                ry= rx*ratio;

            var ellipse= new kity.Ellipse();
            ellipse.setCenter(cx,cy).setRadius(rx,ry)
